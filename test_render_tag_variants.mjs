import crypto from 'node:crypto';

const userProvidedData = "659996538566a34cb7e84323:";
const password = "maggikhalo";

const [ivHex, cipherHex] = userProvidedData.split(':');
const iv = Buffer.from(ivHex, 'hex');
const cipherText = Buffer.from(cipherHex, 'hex');

const keys = [
    crypto.createHash('sha256').update(password).digest(),
    crypto.createHash('md5').update(password).digest(),
    Buffer.alloc(32, password)
];

for (let k of keys) {
    const alg = k.length === 32 ? 'aes-256-gcm' : 'aes-128-gcm';
    
    // Strategy 1: Tag at End (16 bytes)
    try {
        const tag = cipherText.subarray(cipherText.length - 16);
        const encrypted = cipherText.subarray(0, cipherText.length - 16);
        const decipher = crypto.createDecipheriv(alg, k, iv);
        decipher.setAuthTag(tag);
        let dec = decipher.update(encrypted, undefined, 'utf8');
        dec += decipher.final('utf8');
        if (dec.includes('http')) console.log(`STRATEGY-END: ${dec}`);
    } catch(e) {}

    // Strategy 2: Tag at Start (16 bytes)
    try {
        const tag = cipherText.subarray(0, 16);
        const encrypted = cipherText.subarray(16);
        const decipher = crypto.createDecipheriv(alg, k, iv);
        decipher.setAuthTag(tag);
        let dec = decipher.update(encrypted, undefined, 'utf8');
        dec += decipher.final('utf8');
        if (dec.includes('http')) console.log(`STRATEGY-START: ${dec}`);
    } catch(e) {}

    // Strategy 3: No Tag (CBC)
    try {
        const cbcAlg = k.length === 32 ? 'aes-256-cbc' : 'aes-128-cbc';
        // Need 16-byte IV for CBC. Just pad the 12-byte one?
        const cbcIv = Buffer.concat([iv, Buffer.alloc(4)]);
        const decipher = crypto.createDecipheriv(cbcAlg, k, cbcIv);
        let dec = decipher.update(cipherText, undefined, 'utf8');
        dec += decipher.final('utf8');
        if (dec.includes('http')) console.log(`STRATEGY-CBC: ${dec}`);
    } catch(e) {}
}

console.log("Finished tag strategy test.");
