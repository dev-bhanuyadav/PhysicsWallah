...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'DELETE' && (path.startsWith('/api/batches/') || path.startsWith('/api/index.mjs/batches/'))) {
      if (!requireAdmin(req)) return send(res, 401, { error: 'Unauthorized' });
      const id = decodeURIComponent(path.replace(/^\/api\/(index\.mjs\/)?batches\//, ''));
    ...
    if (path.includes('/pw-proxy/')) {
      const targetPath = path.replace(/^\/api\/(index\.mjs\/)?v1\/pw-proxy\//, '');
