import crypto from 'node:crypto';

const userProvidedData = "659996538566a34cb7e84323:";
const password = "maggikhalo";

const [ivHex, cipherHex] = userProvidedData.split(':');
const iv = Buffer.from(ivHex, 'hex');
const cipherText = Buffer.from(cipherHex, 'hex');

function tryDec(key, name) {
    for (let alg of ['aes-256-gcm', 'aes-128-gcm']) {
        try {
            const kLen = alg.includes('256') ? 32 : 16;
            if (key.length >= kLen) {
                const k = key.subarray(0, kLen);
                const tag = cipherText.subarray(cipherText.length - 16);
                const encrypted = cipherText.subarray(0, cipherText.length - 16);
                const decipher = crypto.createDecipheriv(alg, k, iv);
                decipher.setAuthTag(tag);
                let dec = decipher.update(encrypted, undefined, 'utf8');
                dec += decipher.final('utf8');
                if (dec.includes('http')) {
                    console.log(`FOUND! ${name} | Alg: ${alg}`);
                    console.log(dec);
                }
            }
        } catch(e) {}
    }
}

// Try Hex of SHA256
tryDec(Buffer.from(crypto.createHash('sha256').update(password).digest('hex')), "SHA256-HEX");
// Try Hex of MD5
tryDec(Buffer.from(crypto.createHash('md5').update(password).digest('hex')), "MD5-HEX");
// Try maggikhalo directly padded
tryDec(Buffer.alloc(32, password), "Direct Padded");
// Try SHA256 of maggikhalo as binary (already tried, but for completeness)
tryDec(crypto.createHash('sha256').update(password).digest(), "SHA256-BIN");

console.log("Finished complex key test.");
