  ...
  useEffect(() => {
    ...
    const initPlayer = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/v1/media-secure?b=${batchId}&s=${subjectId}&c=${childId}`);
             ...
             if (data.fatal) {
                 switch (data.type) {
                     case window.Hls.ErrorTypes.NETWORK_ERROR:
                         hls.destroy();
                         if (isMounted) {
                           setVideoError(`HLS Network Error: ${data.details}. The video CDN might be blocking access or the tokens expired.`);
                           setVideoLoading(false);
                         }
                         break;
                     ...
                     default:
                         hls.destroy();
                         if (isMounted) {
                           setVideoError(`HLS Crash: ${data.details}`);
                           setVideoLoading(false);
                         }
                         break;
                 }
                 ...
                 if (isMounted && data.details === 'manifestLoadError') {
                    hls.destroy();
                    setVideoError(`HLS Manifest Blocked: ${data.details}. Tokens might be invalid.`);
                    setVideoLoading(false);
                 }
             }
          });
