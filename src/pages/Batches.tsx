...
export default function Batches() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] text-[#1F2937] font-sans">
      <div className="mx-auto max-w-[1400px] min-h-screen flex">
        <aside className="hidden lg:flex w-[270px] bg-white/80 backdrop-blur-xl border-r border-gray-100 px-4 py-6">
          <div className="w-full">
            <div className="flex items-center justify-between px-2">
              <div className="text-sm font-black tracking-wide text-slate-900">LEARN ONLINE</div>
              <button
                onClick={() => navigate('/admin')}
                className="inline-flex items-center justify-center h-9 w-9 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 transition-all"
              >
            ...
            <div className="mt-4 space-y-1">
              {[
                { icon: GraduationCap, label: 'Study', active: false },
                { icon: Pi, label: 'Pi', active: false },
                { icon: Library, label: 'Library', active: false },
              ].map((it) => (
                <button
                  key={it.label}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
                    it.active ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:bg-slate-50'
                  }`}
                >
                  <it.icon size={18} />
            ...
            <div className="mt-6 px-2 text-[11px] font-black tracking-widest text-slate-400 uppercase">
              Study Packs
            </div>
            ...
            <div className="mt-3 space-y-1">
              {[
                ...
                { icon: Zap, label: 'Power Batch', active: false },
                { icon: ClipboardList, label: 'Test Series', active: false },
                { icon: Award, label: 'Scholarship', active: false },
              ].map((it) => (
                <button
                  key={it.label}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
                    it.active ? 'bg-indigo-50 text-indigo-900' : 'text-slate-600 hover:bg-slate-50'
                  }`}
                >
                  <it.icon size={18} className={it.active ? 'text-indigo-700' : undefined} />
                  <span className="text-sm font-bold">{it.label}</span>
                </button>
              ))}
            </div>
            ...
            <div className="mt-3 space-y-1">
              <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-slate-600 hover:bg-slate-50">
                <MapPin size={18} />
                <span className="text-sm font-bold">PW Centres</span>
              </button>
            </div>
          </div>
        </aside>
          ...
          <div className="px-4 sm:px-8 py-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="hidden sm:flex items-center gap-3">
                <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-2 w-[360px] focus-within:border-blue-500/50 transition-all">
                ...
                <button
                  onClick={() => navigate('/admin')}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 transition-all"
                >
                  <Shield size={18} className="text-slate-700" />
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading &&
                Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="rounded-[26px] bg-white border border-slate-200 overflow-hidden h-[460px]"
                  />
                ))}
              {!loading &&
                batches.map((b) => (
